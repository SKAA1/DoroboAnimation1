...
public class NPCControls : MonoBehaviour
{
    ...
    public float escapeDistance = 20; //  Max distance to calcule the a minumun and a maximum raycast when hits something
    ...
    private void Chasing()
    {
        ...
        /* // Calculate the direction from the enemy to the player
         ...
         // Rotate the enemy to face the player's position without looking up
    ...
    {
        if (Vector3.Distance(transform.position, m_PlayerPosition) > escapeDistance)
        {
            // If the enemy has escaped beyond the set distance, make it disappear or handle accordingly.
            // You can deactivate the GameObject, play a disappearance animation, etc.
            gameObject.SetActive(false);
        }
        else
        {
            // Continue running away from the player
            Move(speedRun);
            navMeshAgent.SetDestination(transform.position + (transform.position - m_PlayerPosition).normalized * escapeDistance);
        }
    }
